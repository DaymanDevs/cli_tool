...
def format_wallet_table(wallets):
    ...
    headers = ["Criteria"] + list(wallets.keys())
    criteria_keys = [
        "Mcap", "Liq", "AG", "Bundle", "Funding", "Dev%", "DevBal", "Links",
        "F", "KYC", "Unq", "SM", "X's"
    ]
    ...
    for key in criteria_keys:
        ...
        for name, criteria in wallets.items():
            if key == "Links":
                ...
                value = f"{min_val} - {max_val}" if min_val != float('-inf') or max_val != float('inf') else "Any"
            row.append(value)
            col_widths[name] = max(col_widths[name], len(value))
        rows.append(row)
...
def update_criteria(criteria, wallet_name, existing_names):
    ...
    while True:
        ...
        if choice == "Back" or choice is None:
            ...
            new_name = menu_selection(options, formatted_table, prompt="Enter new wallet name (or blank to cancel): ", allow_input=True)
        ...
        if key == "Links":
            ...
            min_val = menu_selection(options, formatted_table, prompt=f"Enter minimum {key} (or blank for no min): ", allow_input=True)
            if min_val is None:
                continue
